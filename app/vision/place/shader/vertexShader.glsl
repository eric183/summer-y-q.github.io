...
void main() {
  ...
  // float noisePattern = noise(uFrequency * normal + uTime * 0.5);
  float noisePattern = noise(uFrequency * normal);

  float wavePattern = wave(vNormal);
  float displacement = wavePattern * 1.3;
  // float displacement = noisePattern * 0.5;

  vec3 newPosition = vNormal + position + displacement;
  // vec3 newPosition = vNormal + position + noisePattern;
