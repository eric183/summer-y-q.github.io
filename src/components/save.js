import getEchartsInstance from '../renderer/echartsRenderer';
import { getVisualMapMappingData } from '../utils';
import * as conditionInstance from '@/views/edit/utils/condition';
import * as dataformat from '../utils/dataformat';

import * as globalFilter from '@/filters';

import defaultOptions from '../utils/echarts/defaultOptions';
import * as dataLink from '../utils/echarts/dataOptionsLink';
import * as colorOptions from '../utils/echarts/colorOptions';
import * as optionLink from '../utils/echarts/optionsLink';

export default {
    // name: 'line',
    render(containerDom, option, widget) {
        // debugger;
        // option.series.map(x=>{})
        // let _option = {...option};
        option.xAxis = {
        ...
        const render = getEchartsInstance(containerDom, option, widget, this);

        // debugger;
        return render.chart();
    },
    parseOption(data, wLayerMetadata, widget, chartContainer, usePalette) {
        // debugger;
        const chartData = wLayerMetadata;

        /********数据预处理（各种图表处理方式不同，暂无法提取通用） */

        //获取图表字段的配置信息
        const chartCondition = chartData.condition;

        //获取图表字段的相关数据，并对存在数据字典的数据进行数据转换
        const castedKeys = data.xAxis;
        const castedAddtions = data[conditionInstance.CONDITION_ADDITION];
        const castedDatas = data.data;
        const castedSeriesName = data.series;
        const seriesConfig = data.seriesConfig;

        const castedKeysByFormat = dataformat.keyAllDataFormat(castedKeys, chartCondition.xAxis, widget.keymaps);
        const keyNames = [];
        castedKeysByFormat.forEach(element => {
            const keyName = element.join(split$);
            if (!keyNames.includes(keyName)) {
                keyNames.push(keyName);
            }
        });

        const visualMapMappingData = getVisualMapMappingData(chartData, {
            metadata: data,
            keymaps: widget.keymaps,
        ...
        let seriesData = [];
        let aggregateData = [];
        let aggregateSerieDataMap = {};
        const seriesValueNames = [];
        const valueNameMap = [];
        const keyDataStat = [];
        const keyMap = {};

        chartCondition.value.forEach((item, index) => {
            const valueKey = globalFilter.fieldDisplay(item) + split$ + index;
            valueNameMap[valueKey] = index;
            keyDataStat[index] = keyNames.map((item, index) => {
                keyMap[item] = index;
                return { count: 0, total: 0 };
            });
            aggregateData.push([]);
        });
        castedSeriesName.forEach((item, index) => {
            const seriesValueName = item[chartCondition.series.length];

            const valueAxisIndex = seriesConfig[item.join(split$) + split$ + index].valueAxisIndex;
            seriesValueNames.push(seriesValueName + split$ + valueAxisIndex);
        });

        const seriesByFormat = dataformat.keyAllDataFormat(castedSeriesName, chartCondition.series, widget.keymaps);
        const seriesKey = chartCondition.series.map(item => item.col).join(split$);
        const seriesDataMap = {};
        const seriesKeys = [];
        seriesByFormat.forEach((item, index) => {
            const seriesConfigKey = castedSeriesName[index].join(split$) + split$ + index;
            const seriesPrefix = split$ + castedSeriesName[index].slice(0, chartCondition.series.length).join(split$) + split$;
            const valueAxisIndex = seriesConfig[seriesConfigKey].valueAxisIndex;
            const mapKey = seriesKey + seriesPrefix + chartCondition.value[valueAxisIndex]?.col + split$ + valueAxisIndex;
            seriesKeys.push(mapKey);
            seriesDataMap[mapKey] = keyNames.map((item, index) => {
                return { count: 0, total: 0 };
            });
        });

        for (let i = 0, iSize = castedDatas.length; i < iSize; i++) {
            let tempData = [];
            const serieKey = seriesKeys[i];
            for (var j = 0, jSize = castedDatas[i].length; j < jSize; j++) {
                let dimensionValue = castedKeysByFormat[j].join(split$);
                let measureValue = isNaN(castedDatas[i][j]) ? null : parseFloat(castedDatas[i][j]);

                const valueIndex = valueNameMap[seriesValueNames[i]];
                const keyIndex = keyMap[dimensionValue];
                keyDataStat[valueIndex][keyIndex].count++;
                keyDataStat[valueIndex][keyIndex].total += measureValue;
                seriesDataMap[serieKey][keyIndex].count++;
                seriesDataMap[serieKey][keyIndex].total += measureValue;

                let singleData = {
                    name: dimensionValue,
                    metaAddtions: castedAddtions.map(groupAddtion => groupAddtion[i][j]),
                    metaKey: castedKeys[j],
                    seriesKey: castedSeriesName[i].join(split$),
                    //两个字段，0-数据渲染，1-关联visualMap的颜色
                    value: [dimensionValue, measureValue]
                };
                if (visualMapMappingData) {
                    singleData.meta = visualMapMappingData[i][j].key;
                    singleData.value.push(visualMapMappingData[i][j].key);
                }
        ...
        aggregateData = aggregateData.map((item, index) => {
            return keyDataStat[index].map(stat => {
                return stat.total;
            });
        });

        Object.keys(seriesDataMap).forEach(key => {
            aggregateSerieDataMap[key] = seriesDataMap[key].map(stat => {
                return stat.total;
            });
        });
        ...
        const maxData = Math.max(
            ...castedDatas.map(f => {
                f = f.filter(item => typeof item === 'number');
                return Math.max(...f);
            })
        );
        // 取出所有数据最小值
        ...
        //第一步：获取图表默认配置
        let echartOption = defaultOptions.getLineChartOption(seriesData.length, maxData);
        //第二步：与数据关联，更新图表配置（各种图表处理方式不同，暂无法提取通用）
        dataLink.updateLineChartOption(
            echartOption,
            {
                chartCondition,
                castedSeriesName,
                ...
                castedKeysByFormat,
                aggregateData,
                aggregateSerieDataMap,
                castedKeys: castedKeys.map(key => key.join(split$)),
                keyNames,
                keymaps: widget.keymaps,
                max: maxData,
                min: minData
        ...
        // 第三步：配色方案适配
        colorOptions.echartsColorFill(chartData, echartOption, visualMapMappingData, usePalette);

        //第四步：与配置项关联，更新图表配置（各种图表处理方式不同，暂无法提取通用）
        optionLink.updateLineChartOption(chartData, echartOption, chartContainer, { chartCondition, seriesConfig, keymaps: widget.keymaps });
